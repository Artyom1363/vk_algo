...
const unsigned int TIMES_CODING = 10;
...
struct ServiceInfo {
    ...
    vector <byte> createVector() {
        ...
        byte allFreeBits = (freeBitsAfterInfo << 4);
        allFreeBits |= freeBits;
        ans.push_back(allFreeBits);
        return ans;
    }
    ...
    ServiceInfo(InpBitStream &inpBitStream) {
        ...
        byte allFreeBits = inpBitStream.ReadByte();
        freeBits = (allFreeBits & 15);
        freeBitsAfterInfo = (allFreeBits >> 4);
        // unsigned int sizeOf
        dataBitsCount = inpBitStream.GetSize() * 8 - sizeOfServiceInfo * 8 - freeBits - 24;
        // inpBitStream.GetSize() * 8 - 
        // sizeOfServiceInfo * 8 размер сервисной инфы в битах
        // freeBits кол-во пустых быт
        // 24 - суммарный размер полей в битах
    }
...
void Encode(IInputStream& original, IOutputStream& compressed) {
    ...
    vector <byte> testBuffer = buffer;
    bool flag = false;
    for (byte i = 1; i <= TIMES_CODING; ++i) {
        HuffmanTree ht(testBuffer);
        ht.BuildTree();
        testBuffer = ht.PackInfo();
        if (!flag || (testBuffer.size() < buffer.size())) {
            flag = true;
            buffer = testBuffer;
            buffer.push_back(i);
            // cout << "packed: " << (int)i << " times" << endl;
        }
...
void Decode(IInputStream& compressed, IOutputStream& original) {
    ...
    // cout << "\nend of reading\n";
    if (DEBUG) cout << "unpacked" << endl << buffer.size() << endl;
    for (auto sym : buffer) {
        if (DEBUG) cout << sym;
        original.Write(sym);
    }
